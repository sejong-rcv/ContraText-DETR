...
class TextDetEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        self._logger.info(f"det_method results: {text_result['det_method']}")
        
        try:
            det_str = text_result['det_method']
            numbers = re.findall(r'\d+\.\d+', det_str) 
            self._logger.info(f"Extracted det numbers: {numbers}")
            
            if len(numbers) >= 3:
                precision = float(numbers[0])
                recall = float(numbers[1])
                hmean = float(numbers[2])
                
                self._results["DET_RESULT"] = {
                    "precision": precision,
                    "recall": recall,
                    "hmean": hmean
                }
                self._logger.info(f"Final det results: {self._results['DET_RESULT']}")
            else:
                raise ValueError("Not enough numbers found in det string")
                
        except Exception as e:
            self._logger.error(f"Error in det_method parsing: {str(e)}")
            self._results["DET_RESULT"] = {
                "precision": 0.0,
                "recall": 0.0,
                "hmean": 0.0
    ...
    def instances_to_coco_json(self, instances, img_id, img_name):
        ...
        if self.use_polygon:
            try:
                ...
                pnts = instances.polygons.to(self._cpu_device).numpy()
            except:
                print(instances)
                breakpoint()
        else:
