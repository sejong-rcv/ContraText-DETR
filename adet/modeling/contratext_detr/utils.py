...
class MLP_v2(nn.Module):
    ...
    def forward(self, x, return_hidden=False):
        hidden_features = []  # List to store hidden features
        for i, layer in enumerate(self.layers):
            x = F.relu(layer(x)) if i < self.num_layers - 1 else layer(x)
            if i < self.num_layers - 1 and return_hidden:  # Save hidden features
                hidden_features.append(x)
        if return_hidden:
            return x, hidden_features  # Return final output and hidden features
        return x
