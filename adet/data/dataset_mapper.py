...
from .transforms.low_contrast_textseg import synthesize_textseg
from .transforms.mask_utils import MaskApplier
...
class DatasetMapperWithBasis(DatasetMapper):
    ...
    def __init__(self, cfg, is_train=True):
        ...
        self.mask_applier = MaskApplier(cfg)
    ...
    def __call__(self, dataset_dict):
        ...
        image, dataset_dict = self.mask_applier.apply_black_mask(image, dataset_dict, self.is_train)

        if self.is_train:
            if random.random() < 0.1:
                image, ann_list = synthesize_textseg(image, boxes, self.cfg)
                dataset_dict["annotations"].extend(ann_list)
