...
from adet.modeling.contratext_detr.utils import MLP, gen_point_pos_embed
...
class DeformableTransformer_Det(nn.Module):
    ...
    def gen_encoder_output_proposals(self, memory, memory_padding_mask, spatial_shapes):
        N_, S_, C_ = memory.shape # N_ -> batch size
        base_scale = 4.0
        proposals = []
        _cur = 0
        for lvl, (H_, W_) in enumerate(spatial_shapes):
            ...
            grid = (grid.unsqueeze(0).expand(N_, -1, -1, -1) + 0.5) / scale  # grid noramlization
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed):
        ...
        # encoder
        memory, encoder_attn_maps = self.encoder(
            src_flatten,
            spatial_shapes,
            level_start_index,
            valid_ratios,
        ...
        init_reference_out = reference_points  # [bs, 100, 16, 2]
        # learnable control point content queries
        query_embed = query_embed.unsqueeze(0).expand(bs, -1, -1, -1)

        hs, inter_references, decoder_attn_maps = self.decoder(
            query_embed,
            reference_points,
            memory,
            spatial_shapes,
        ...
        return hs, init_reference_out, inter_references_out, enc_outputs_class, enc_outputs_coord_unact, encoder_feat, encoder_attn_maps, decoder_attn_maps
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        ...
        for _, layer in enumerate(self.layers):
            output, attn_map = layer(output, pos, reference_points, spatial_shapes, level_start_index, padding_mask)
    ...
    def forward(
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.epqm:
                ...
                query_pos = gen_point_pos_embed(reference_points_input[:, :, :, 0, :]) # [bs, 100, 16, n_dim(256)]
                # get the positional queries
                query_pos = self.ref_point_head(query_pos) # projection  FFN
        ...
        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(intermediate_reference_points), attn_maps

        return output, reference_points, attn_maps
